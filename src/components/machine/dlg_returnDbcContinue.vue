  ...
  <el-dialog :visible.sync="isOpen" width="540px" @close="closed">
    ...
    <div class="dlg-content">
      <div class="midInfo mt20">
        <p v-if="item">{{$t('gpu.backDbcNum')}}: {{item.dbc_total_count}}DBC</p>
        <div>
          <span v-html></span>
          <span></span>
        </div>
...
<script>
import { continue_pay_return_dbc, continue_pay_return_dbc_code } from "@/api";

export default {
  name: "popupReturnContinue",
  props: {
    open: Boolean,
    item: Object
  },
  ...
  methods: {
    ...
    getCode() {
      ...
      continue_pay_return_dbc_code({
        continue_pay_order_id: this.item.continue_pay_order_id,
        user_name_platform,
        language
      })
        .then(res => {
