<template>
  <el-dialog :visible.sync="isOpen" width="540px" @close="closed">
    <div slot="title">{{$t('my_machine_will_close')}}</div>
      ...
      <div class="midInfo mt20">
        ...
        <el-form>
          <el-form-item :label="$t('dlg_input_code')">
            <el-input v-model="sold_out_code" size="small" style="width: 200px"></el-input>
            <el-button
              :loading="isCoding"
              class="ml10"
              plain
              size="mini"
              @click="get_sold_out_machine_code"
...
<script>
import { sold_out_machine, get_sold_out_machine_code } from "@/api";
...
export default {
  ...
  methods: {
    ...
    get_sold_out_machine_code() {
      this.isCoding = true;
      const user_name_platform = this.$t("website_name");
      const language = this.$i18n.locale;
      get_sold_out_machine_code({
        machine_id: this.item.machine_id,
